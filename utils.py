...
def plotClassification(x, y, ax):
    # Création des couleurs pour chaque classe (1 et -1)
    colors = {1: 'blue', -1: 'orange'}

    # Affichage des points avec des couleurs correspondant aux classes
    for cls in np.unique(y):
        ax.scatter(x[y == cls, 0], x[y == cls, 1], 
                   label=f'class {cls}', 
                   color=colors[cls], alpha=0.7, s=50)
