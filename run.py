...
for model_class in ['krr', 'log_reg', 'svc']:  # 'svc', 'krr', 'wkrr', 'log_reg'
    for encoding in ['bin' , 'int', 'one_hot']: 
        best_params_tot = {}
        ...
        for kernel_type in ['Sigmoid','linear', 'gaussian', 'Poly']:  # 'linear', 'gaussian', 'Poly', 'Sigmoid'
            ...
            print(f"Model class: {model_class}, encoding: {encoding}, kernel type: {kernel_type}, params: {params_grid}")
            for k in range(3):
